...
class YtDlpService {
  ...
  runCommand(args, maxBuffer = 10 * 1024 * 1024) {
    return new Promise((resolve, reject) => {
      ...
      ytdlpProcess.stdout.on('data', (data) => {
        ...
        if (bufferSize > maxBuffer) {
          ytdlpProcess.kill();
          return reject(new Error('Process buffer exceeded maxBuffer size'));
        }
  ...
  async getAudioUrl(videoId) {
    try {
      ...
      const stdout = await this.runCommand(args, 1024 * 1024);
  ...
  async getAudioFormats(videoId) {
    try {
      ...
      const stdout = await this.runCommand(args, 5 * 1024 * 1024);
  ...
  async downloadAudio(videoId, outputPath) {
    try {
      ...
      await this.runCommand(args, 50 * 1024 * 1024);
  ...
  async getPlaylist(playlistId) {
    try {
      ...
      const stdout = await this.runCommand(args, 20 * 1024 * 1024);
