...
class YtDlpService {
  ...
  // Securely execute yt-dlp command using spawn
  _executeYtDlp(args, options = {}) {
    return new Promise((resolve, reject) => {
      const ytdlpProcess = spawn(this.ytdlpPath, args, options);
      let stdout = '';
      let stderr = '';
  ...
  async search(query, limit = 20) {
    try {
      ...
      const stdout = await this._executeYtDlp(args, { maxBuffer: 10 * 1024 * 1024 });
  ...
  async getAudioUrl(videoId) {
    try {
      ...
      const stdout = await this._executeYtDlp(args, { maxBuffer: 1024 * 1024 });
  ...
  async getAudioFormats(videoId) {
    try {
      ...
      const stdout = await this._executeYtDlp(args, { maxBuffer: 5 * 1024 * 1024 });
  ...
  async downloadAudio(videoId, outputPath) {
    try {
      ...
      await this._executeYtDlp(args, { maxBuffer: 50 * 1024 * 1024 });
  ...
  async getTrending(region = 'US') {
    try {
      // Note: Region is not directly used in a way that allows injection here, but this is kept for consistency.
  ...
  async getPlaylist(playlistId) {
    try {
      ...
      const stdout = await this._executeYtDlp(args, { maxBuffer: 20 * 1024 * 1024 });
  ...
  async checkYtDlp() {
    try {
      const args = ['--version'];
      const stdout = await this._executeYtDlp(args);
