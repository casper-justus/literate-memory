...
class YtDlpService {
  ...
  async search(query, limit = 20) {
    try {
      ...
      const { stdout } = await spawnAsync(this.ytdlpPath, args, { maxBuffer: 10 * 1024 * 1024 });
  ...
  async getAudioUrl(videoId) {
    try {
      ...
      const { stdout } = await spawnAsync(this.ytdlpPath, args, { maxBuffer: 1024 * 1024 });
  ...
  async getAudioFormats(videoId) {
    try {
      ...
      const { stdout } = await spawnAsync(this.ytdlpPath, args, { maxBuffer: 5 * 1024 * 1024 });
  ...
  async downloadAudio(videoId, outputPath) {
    try {
      ...
      await spawnAsync(this.ytdlpPath, args, { maxBuffer: 50 * 1024 * 1024 });
  ...
  async getPlaylist(playlistId) {
    try {
      ...
      const { stdout } = await spawnAsync(this.ytdlpPath, args, { maxBuffer: 20 * 1024 * 1024 });
